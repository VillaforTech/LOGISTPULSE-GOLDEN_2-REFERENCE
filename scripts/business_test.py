...
p=argparse.ArgumentParser();p.add_argument('--base',default=os.getenv('BASE_URL','http://localhost:28080'));p.add_argument('--output',default='artifacts/business');a=p.parse_args()
out=Path(a.output);out.mkdir(parents=True,exist_ok=True)
fixture='business-'+str(uuid.uuid4());evidence={'fixtureRunId':fixture,'startedAt':time.time(),'observations':[],'technical':{},'expected':{'status':'READY','deadlineSeconds':15,'total':'25.50','unit':'DEMO'}}
...
try:
    for service in ('inventory','logistics','operations','fulfillment','worker','relay','analytics','live'):
        evidence['technical'][service]=get('/health/'+service)
        assert evidence['technical'][service]['status']=='UP'
    response=client.post('/api/fulfillment/orders',json={'storeId':'STORE-042','channel':'BUSINESS-CI','total':'25.50','fixtureRunId':fixture},headers={'X-Correlation-ID':fixture})
    response.raise_for_status();order=response.json();evidence['created']=order;save()
    deadline=time.monotonic()+22
    while time.monotonic()<deadline:
        observed=get('/api/fulfillment/orders/'+order['orderId']);snap=get('/api/business/snapshot?fixtureRunId='+fixture)
        evidence['observations'].append({'at':time.time(),'order':observed,'snapshot':snap})
    ...
    assert observed['status']=='READY',f"Business breach: {observed['status']} after {time.time()-order['createdAt']:.2f}s; health does not prove fulfillment"
    assert observed['readyAt'] is not None
    assert 0 <= observed['readyAt']-observed['createdAt'] <= 15,'READY missed 15-second promise'
    ...
    evidence.update(passed=False,error=str(exc),completedAt=time.time());save()
    print(f'Business contract BLOCK: {exc}',file=sys.stderr);sys.exit(1)
