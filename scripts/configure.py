...
import os, secrets
path=Path('.env')
values=dict(line.split('=',1) for line in path.read_text().splitlines() if '=' in line and not line.startswith('#')) if path.exists() else {}
...
if os.getenv('CODESPACE_NAME'):
    values.setdefault('PUBLIC_BASE_URL',f"https://{os.environ['CODESPACE_NAME']}-28080.{os.getenv('GITHUB_CODESPACES_PORT_FORWARDING_DOMAIN','app.github.dev')}")
