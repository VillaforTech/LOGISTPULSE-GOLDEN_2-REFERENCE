...
from .domain import FACTS_TOPIC, calculate, alert_states, epoch, utc, validate_event, coverage_digest, decode_message
...
state = {'ready': False, 'lastPoll': 0., 'sourceCheckedAt': 0., 'sourcePosition': 0, 'lag': 0, 'sourcePending':0, 'aggregateCount':0, 'versionDigest':None}
DUPLICATES = Counter('logistpulse_analytics_duplicates_total', 'Ignored duplicate event identities')
ERRORS = Counter('logistpulse_analytics_errors_total', 'Invalid events quarantined')
GAUGES = {name: Gauge('logistpulse_' + name, help) for name, help in {
    'orders_due_window':'Eligible orders in 15 minute cohort', 'orders_breached_window':'Orders missing promise in cohort',
    'overdue_order_value':'Overdue backlog value in demo units', 'preparation_debt_seconds':'Overdue backlog order seconds',
    'analytics_lag':'Kafka records behind end offset', 'analytics_gaps':'Unresolved aggregate revision gaps',
    'analytics_snapshot_timestamp_seconds':'Last committed snapshot time', 'analytics_timer_lag_seconds':'Timer scheduling delay',
...
def set_meta(db, key, value):
    db.execute('INSERT INTO analytics_meta VALUES (%s,%s) ON CONFLICT(key) DO UPDATE SET value=EXCLUDED.value', (key, Jsonb(value)))


def migrate():
    def run():
        with dbconn() as db:
            db.execute('''CREATE TABLE IF NOT EXISTS projection(order_id text PRIMARY KEY, version integer NOT NULL,
                data jsonb NOT NULL, deadline_at numeric NOT NULL)''')
            db.execute('''CREATE TABLE IF NOT EXISTS processed_events(event_id text PRIMARY KEY, aggregate_id text,
                aggregate_version integer, envelope jsonb, received_at numeric)''')
            db.execute('CREATE UNIQUE INDEX IF NOT EXISTS unique_aggregate_version ON processed_events(aggregate_id,aggregate_version)')
            db.execute('''CREATE TABLE IF NOT EXISTS consumer_offsets(topic text, partition integer,
                next_offset bigint, PRIMARY KEY(topic,partition))''')
            db.execute('CREATE TABLE IF NOT EXISTS revision_gaps(order_id text, version integer, PRIMARY KEY(order_id,version))')
            db.execute('''CREATE TABLE IF NOT EXISTS quarantine(topic text, partition integer, record_offset bigint,
                reason text, envelope jsonb, PRIMARY KEY(topic,partition,record_offset))''')
            db.execute('CREATE TABLE IF NOT EXISTS analytics_meta(key text PRIMARY KEY,value jsonb)')
            db.execute('CREATE TABLE IF NOT EXISTS snapshots(id integer PRIMARY KEY CHECK(id=1), revision bigint, snapshot jsonb)')
            db.execute('INSERT INTO snapshots VALUES (1,0,%s) ON CONFLICT DO NOTHING', (Jsonb({}),))
            db.execute('''CREATE TABLE IF NOT EXISTS alerts(kpi text PRIMARY KEY, active boolean NOT NULL,
                activated_at numeric, resolved_at numeric)''')
            db.execute('''CREATE TABLE IF NOT EXISTS alert_history(id bigserial PRIMARY KEY, kpi text,active boolean,
...
def bootstrap_projection():
    ...
    with dbconn() as db:
        for order in source['orders']:
            db.execute('''INSERT INTO projection VALUES (%s,%s,%s,%s)
                ...
                (order['orderId'], order['version'], Jsonb(order), epoch(order['createdAt']) + 15))
...
def persist_event(event, topic, partition, offset):
    ...
    with dbconn() as db:
        if reason:
            db.execute('INSERT INTO quarantine VALUES (%s,%s,%s,%s,%s) ON CONFLICT DO NOTHING',
                       (topic, partition, offset, reason, Jsonb(event)))
            ERRORS.inc()
        else:
            inserted = db.execute('''INSERT INTO processed_events VALUES (%s,%s,%s,%s,%s)
                ON CONFLICT DO NOTHING RETURNING event_id''',
                (event['eventId'],event['aggregateId'],event['aggregateVersion'],Jsonb(event),time.time())).fetchone()
            if inserted:
                oid, version = event['aggregateId'], event['aggregateVersion']
                current = db.execute('SELECT version FROM projection WHERE order_id=%s FOR UPDATE', (oid,)).fetchone()
                previous = current['version'] if current else 0
                if version > previous + 1000:
                    db.execute('INSERT INTO quarantine VALUES (%s,%s,%s,%s,%s) ON CONFLICT DO NOTHING',
                               (topic,partition,offset,'Unbounded version gap',Jsonb(event)))
                else:
                    for missing in range(previous + 1, version):
                        db.execute('INSERT INTO revision_gaps VALUES (%s,%s) ON CONFLICT DO NOTHING', (oid,missing))
                    db.execute('DELETE FROM revision_gaps WHERE order_id=%s AND version=%s', (oid,version))
                    if version > previous:
                        data = event['data']
                        db.execute('''INSERT INTO projection VALUES (%s,%s,%s,%s)
                            ON CONFLICT(order_id) DO UPDATE SET version=EXCLUDED.version,data=EXCLUDED.data,deadline_at=EXCLUDED.deadline_at''',
                            (oid,version,Jsonb(data),epoch(data['createdAt'])+15))
                        set_meta(db, 'sourceEvent', {key:event.get(key) for key in
                            ('eventId','aggregateId','aggregateVersion','correlationId','occurredAt','sourcePosition')})
                    set_meta(db, 'lastPosition', max(metadata(db,'lastPosition',0), event.get('sourcePosition',0)))
            else:
                previous = db.execute('SELECT envelope FROM processed_events WHERE event_id=%s OR (aggregate_id=%s AND aggregate_version=%s)',
                    (event['eventId'],event['aggregateId'],event['aggregateVersion'])).fetchall()
                if len(previous) != 1 or previous[0]['envelope'] != event:
                    db.execute('INSERT INTO quarantine VALUES (%s,%s,%s,%s,%s) ON CONFLICT DO NOTHING',
                        (topic,partition,offset,'Conflicting event identity or aggregate version',Jsonb(event)))
                    ERRORS.inc()
                else:
                    DUPLICATES.inc()
        db.execute('''INSERT INTO consumer_offsets VALUES (%s,%s,%s) ON CONFLICT(topic,partition)
            DO UPDATE SET next_offset=GREATEST(consumer_offsets.next_offset,EXCLUDED.next_offset)''', (topic,partition,offset+1))
...
def snapshot_data(db, now, fixture=None):
    ...
    offsets = db.execute('SELECT topic,partition,next_offset FROM consumer_offsets ORDER BY topic,partition').fetchall()
    coverage = metadata(db,'coverage',{'complete':False, 'reason':'Bootstrap pending'})
    ...
    return {'schemaVersion':1, 'computedAt':utc(now), 'generatedAt':utc(now),
...
def commit_snapshot():
    ...
    with dbconn() as db:
        ...
        if data['quality']['status'] == 'FRESH':
            for key, active in alert_states(data['kpis']).items():
                old = db.execute('SELECT active FROM alerts WHERE kpi=%s', (key,)).fetchone()
                if old is None or old['active'] != active:
                    db.execute('''INSERT INTO alerts VALUES (%s,%s,%s,%s) ON CONFLICT(kpi) DO UPDATE
                    ...
                    db.execute('INSERT INTO alert_history(kpi,active,computed_at,snapshot_revision) VALUES (%s,%s,%s,%s)', (key,active,now,revision))
        data['alerts'] = db.execute('SELECT kpi,active,activated_at,resolved_at FROM alerts ORDER BY kpi').fetchall()
        ...
        db.execute('UPDATE snapshots SET revision=%s,snapshot=%s WHERE id=1', (revision,Jsonb(data)))
    ...
    GAUGES['orders_due_window'].set(data['kpis']['L-K1']['sample'] if valid else float('nan'))
    GAUGES['orders_breached_window'].set(data['kpis']['L-K1']['breached'] if valid else float('nan'))
    GAUGES['overdue_order_value'].set(float(data['kpis']['L-K2']['value']) if valid else float('nan'))
    GAUGES['preparation_debt_seconds'].set(data['kpis']['L-K3']['value'] if valid else float('nan'))
    ...
    def on_partitions_assigned(self, partitions):
        with dbconn() as db:
            for tp in partitions:
                row = db.execute('SELECT next_offset FROM consumer_offsets WHERE topic=%s AND partition=%s', (tp.topic,tp.partition)).fetchone()
                if row:
...
def consume():
    while not stop.is_set():
        ...
        try:
            ...
            consumer = KafkaConsumer(bootstrap_servers=os.environ['KAFKA_BOOTSTRAP'],
                group_id='logistpulse-business-analytics-v1', enable_auto_commit=False, auto_offset_reset='earliest',
                max_poll_records=1)
            consumer.subscribe([FACTS_TOPIC],listener=CheckpointListener(consumer))
            while not stop.is_set():
                batches = consumer.poll(timeout_ms=100, max_records=1)
                for messages in batches.values():
                    for message in messages:
                        persist_event(decode_message(message.value),message.topic,message.partition,message.offset)
                        consumer.commit()
                end = consumer.end_offsets(consumer.assignment()) if consumer.assignment() else {}
                state.update(ready=bool(consumer.assignment()), lastPoll=time.time(),
                             lag=sum(max(0,offset-consumer.position(tp)) for tp,offset in end.items()))
                if time.time()-state['sourceCheckedAt'] >= .5:
                    response = httpx.get(SOURCE + '/api/fulfillment/watermark', timeout=2)
                    response.raise_for_status()
                    mark = response.json()
                    state.update(sourcePosition=mark['sourcePosition'],sourcePending=mark['pending'],
                                 aggregateCount=mark['aggregateCount'],versionDigest=mark['versionDigest'],sourceCheckedAt=time.time())
...
@app.get('/health')
def health():
    with dbconn() as db: db.execute('SELECT 1 FROM snapshots')
...
def age_checked(snapshot, now):
    ...
    snapshot['quality'] = {'status':'STALE','reason':'Snapshot timer heartbeat expired'}
...
def projected(order_id: str):
    with dbconn() as db:
        row = db.execute('SELECT data FROM projection WHERE order_id=%s',(order_id,)).fetchone()
    if row is None: raise HTTPException(404,'Order not projected')
    return row['data']
...
def alerts():
    with dbconn() as db:
        return db.execute('SELECT * FROM alert_history ORDER BY id DESC LIMIT 100').fetchall()
