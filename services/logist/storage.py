...
def connect(variable='FULFILLMENT_DB_URL'):
    return psycopg.connect(os.environ[variable], row_factory=dict_row, connect_timeout=3)
...
def migrate():
    def run():
        with connect() as db:
            ...
            for declaration in ('ready_at numeric', 'version integer NOT NULL DEFAULT 1', 'fixture_run_id text',
                                'correlation_id text', 'preparing_at numeric'):
            ...
            db.execute('CREATE INDEX IF NOT EXISTS outbox_pending ON outbox(sequence) WHERE published_at IS NULL')
            db.execute('''CREATE TABLE IF NOT EXISTS kitchen_inbox (
...
def snapshot(row):
    return {
        'orderId': row['order_id'], 'storeId': row['store_id'], 'channel': row['channel'],
        'total': format(row['total'], '.2f'), 'unit': 'DEMO', 'status': row['status'],
        'createdAt': utc(row['created_at']), 'updatedAt': utc(row['updated_at']),
        'readyAt': utc(row['ready_at']) if row['ready_at'] is not None else None,
        'version': row['version'], 'fixtureRunId': row['fixture_run_id'],
...
def api_view(row):
    ...
    result['total'] = float(row['total'])  # Existing UI compatibility; events retain decimal strings.
    for key, column in [('createdAt', 'created_at'), ('updatedAt', 'updated_at'), ('readyAt', 'ready_at')]:
