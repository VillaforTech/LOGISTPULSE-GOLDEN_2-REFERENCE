...
from .storage import COLUMNS, connect, migrate, record_transition, snapshot, update_order
...
def process(command, command_id):
    oid = command['orderId']
    with connect() as db:
        if db.execute('SELECT 1 FROM kitchen_inbox WHERE command_id=%s', (command_id,)).fetchone():
            return
        row = db.execute('SELECT ' + COLUMNS + ' FROM orders WHERE order_id=%s FOR UPDATE', (oid,)).fetchone()
        if row is None:
            raise ValueError('Command references a missing order')
        if row['status'] == 'READY':
            db.execute('INSERT INTO kitchen_inbox VALUES (%s,%s,%s) ON CONFLICT DO NOTHING', (command_id, oid, time.time()))
            return
    ...
    if stop.wait(max(0, PREPARATION_SECONDS - (time.time() - started))):
        raise RuntimeError('Worker stopping before completion; command remains uncommitted')
    with connect() as db:
        row = db.execute('SELECT ' + COLUMNS + ' FROM orders WHERE order_id=%s FOR UPDATE', (oid,)).fetchone()
...
def run():
    while not stop.is_set():
        consumer = None
        try:
            consumer = KafkaConsumer(COMMANDS_TOPIC, bootstrap_servers=os.environ['KAFKA_BOOTSTRAP'],
                group_id='kitchen-worker', enable_auto_commit=False, auto_offset_reset='earliest',
                value_deserializer=lambda b: json.loads(b.decode()), max_poll_records=1)
            while not stop.is_set():
                ...
                for messages in batches.values():
                    for message in messages:
                        ...
                        command_id = command.get('commandId') or f'legacy:{message.topic}:{message.partition}:{message.offset}'
