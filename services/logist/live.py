...
def line_protocol(snapshot):
    ...
    def value(key):
        ...
        if quality != 'FRESH' or item['status'] in ('STALE','INCOMPLETE'): return '-1'
        if item['status'] == 'NO_SAMPLE': return '-2'
        return str(item['value'])
    fields = {f'lk{i}':value(f'L-K{i}') for i in (1,2,3)}
    fields.update(revision=str(snapshot['revision'])+'i', sample=str(snapshot['kpis']['L-K1']['sample'])+'i',
    ...
    for key, val in [('correlation_id',snapshot.get('correlationId') or ''),
    ...
    return MEASUREMENT + ' ' + ','.join(k+'='+v for k,v in fields.items()) + ' ' + str(time.time_ns())
        ...
        while not stop.is_set():
            ...
            try:
                ...
                if candidate['revision'] >= state['revision']:
                    ...
                    publish(client,last)
                ws = websocket.create_connection(ANALYTICS.replace('http','ws',1)+'/api/business/ws',timeout=2)
                while not stop.is_set():
                ...
                if last:
                    stale = dict(last,quality={'status':'STALE','reason':'Analytics stream disconnected'})
                    try: publish(client,stale)
