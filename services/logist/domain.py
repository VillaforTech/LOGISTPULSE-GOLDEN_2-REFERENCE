...
DEADLINE_SECONDS = 15
WINDOW_SECONDS = 900
EVENT_STATES = {'ORDER_CREATED': 'WAITING', 'ORDER_PREPARING': 'PREPARING', 'ORDER_READY': 'READY'}
...
def utc(epoch):
    return datetime.fromtimestamp(float(epoch), timezone.utc).isoformat(timespec='microseconds').replace('+00:00', 'Z')
...
def event_for(order, event_type, now):
    event = {
        'schemaVersion': 1, 'eventId': str(uuid.uuid4()), 'eventType': event_type,
        'aggregateType': 'Order', 'aggregateId': order['orderId'],
        'aggregateVersion': order['version'], 'occurredAt': utc(now),
        'correlationId': order['correlationId'], 'fixtureRunId': order.get('fixtureRunId'),
        'data': dict(order),
    }
...
def validate_event(event):
    ...
    if event.get('aggregateType') != 'Order' or event.get('eventType') not in EVENT_STATES:
    ...
    data = event['data']
    if not event.get('aggregateId') or data['orderId'] != event['aggregateId'] or data['version'] != version:
        raise ValueError('aggregate identity/version mismatch')
    if (not isinstance(event.get('correlationId'),str) or not event['correlationId']
        or data.get('correlationId') != event['correlationId'] or data.get('unit') != 'DEMO'
    ...
    if (data['status'] == 'READY') != (ready is not None):
        raise ValueError('READY requires an observed readyAt; other states must not have it')
    if ready is not None and (epoch(ready) < created or epoch(ready) > epoch(data['updatedAt']) or epoch(ready) > epoch(event['occurredAt'])):
...
def transition(order, target, now):
    ...
    if order['status'] == target or order['status'] == 'READY':
        return order, False
    if (order['status'], target) not in {('WAITING', 'PREPARING'), ('PREPARING', 'READY')}:
        raise ValueError('invalid order transition')
    result = dict(order, status=target, version=order['version'] + 1, updatedAt=utc(now))
    if target == 'READY':
...
def calculate(orders, now, *, window=WINDOW_SECONDS, deadline=DEADLINE_SECONDS):
    ...
    cohort = [o for o in orders if now - window < epoch(o['createdAt']) <= now - deadline]
    late = [o for o in cohort if o.get('readyAt') is None or epoch(o['readyAt']) > epoch(o['createdAt']) + deadline]
    overdue = [o for o in orders if o['status'] in ('WAITING', 'PREPARING') and epoch(o['createdAt']) + deadline <= now]
    value = sum((Decimal(o['total']) for o in overdue), Decimal('0'))
    debt = sum(max(0, now - epoch(o['createdAt']) - deadline) for o in overdue)
    return {
        'L-K1': {'value': 100 * len(late) / len(cohort) if cohort else None, 'unit': 'percent',
                 'status': 'OK' if cohort else 'NO_SAMPLE', 'sample': len(cohort), 'breached': len(late)},
        'L-K2': {'value': format(value, '.2f'), 'unit': 'DEMO', 'status': 'OK', 'sample': len(overdue)},
        'L-K3': {'value': round(debt, 6), 'unit': 'order_seconds', 'status': 'OK', 'sample': len(overdue)},
    }


def alert_states(kpis):
    return {key: value['value'] is not None and Decimal(str(value['value'])) > 0 for key, value in kpis.items()}
...
def decode_message(raw):
    try:
        ...
        return {'_malformed': True, 'sha256': hashlib.sha256(raw).hexdigest(),
