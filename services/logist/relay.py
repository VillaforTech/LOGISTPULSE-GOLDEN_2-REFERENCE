...
def setup_topics():
    admin = KafkaAdminClient(bootstrap_servers=os.environ['KAFKA_BOOTSTRAP'], request_timeout_ms=5000)
    try:
...
def relay_once(producer):
    with connect() as db:
        rows = db.execute('''SELECT sequence,topic,aggregate_id,envelope FROM outbox
            WHERE published_at IS NULL ORDER BY sequence LIMIT 30 FOR UPDATE SKIP LOCKED''').fetchall()
        for row in rows:
            producer.send(row['topic'], key=row['aggregate_id'].encode(), value=row['envelope']).get(timeout=10)
            db.execute('UPDATE outbox SET published_at=%s,attempts=attempts+1 WHERE sequence=%s', (time.time(), row['sequence']))
    return len(rows)
...
def run():
    ...
    while not stop.is_set():
        try:
            if producer is None:
                setup_topics()
                producer = KafkaProducer(bootstrap_servers=os.environ['KAFKA_BOOTSTRAP'], acks='all',
                    retries=3, max_in_flight_requests_per_connection=1, request_timeout_ms=5000,
                    max_block_ms=5000, value_serializer=lambda v: json.dumps(v).encode())
