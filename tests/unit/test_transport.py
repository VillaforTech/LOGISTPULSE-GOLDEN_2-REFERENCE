...
def sample():
    return dict(computedAt=utc(1000),revision=1,quality={'status':'FRESH','reason':''},
                coverage={'complete':True},kpis=calculate([],1000))
...
def test_checkpoint_restores_every_assignment(monkeypatch):
    db=Mock(); db.execute.return_value.fetchone.return_value={'next_offset':42}
    cm=Mock();cm.__enter__=Mock(return_value=db);cm.__exit__=Mock(return_value=False)
    monkeypatch.setattr(analytics,'dbconn',lambda:cm)
    consumer=Mock(); listener=analytics.CheckpointListener(consumer)
