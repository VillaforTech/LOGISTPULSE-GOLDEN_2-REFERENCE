...
import json
import pytest
from logist.domain import (money,utc,epoch,event_for,validate_event,transition,calculate,
                           coverage_digest,decode_message,alert_states)
from logist.worker import finish_preparation


def order(created=1000,status='WAITING',ready=None,total='25.50',version=1):
    return {'orderId':'ORD-1','storeId':'STORE-042','channel':'TEST','total':total,'unit':'DEMO',
            'status':status,'createdAt':utc(created),'updatedAt':utc(ready or created),
            'readyAt':utc(ready) if ready is not None else None,'version':version,
            'correlationId':'run-1','fixtureRunId':'fixture-1'}


@pytest.mark.parametrize('value',['NaN','Infinity','-1','0','1.234','999999999999.99'])
...
def test_lifecycle_and_repeated_ready():
    ...
    with pytest.raises(ValueError): transition(initial,'READY',1001)
...
def test_late_delivery_of_on_time_ready_corrects_provisional_breach():
    ...
    assert calculate([order(status='READY',ready=1014,version=3)],1021)['L-K1']['value']==0
...
def test_event_contract_does_not_hide_real_breach():
    ...
    for key,value in [('schemaVersion',2),('aggregateVersion',0),('aggregateId','other')]:
        broken=deepcopy(event);broken[key]=value
        with pytest.raises(ValueError): validate_event(broken)


def test_ready_event_requires_observed_timestamp():
    with pytest.raises(ValueError): event_for(order(status='READY'),'ORDER_READY',1002)
